...
export default function ProcessFlowOutreach({ lang }) {
  const isRtl = lang === 'ar';
  
  const steps = isRtl 
    ? ['إرسال (Send)', 'سلسلة (Sequence)', 'رسالة أولى (Icebreaker)', 'بحث (Research)', 'إيجاد (Find)']
    : ['Find', 'Research', 'Icebreaker', 'Sequence', 'Send'];
          ...
          <g key={i} transform={`translate(${i * 150 + 50}, 60)`}>
            ...
            {i < steps.length - 1 && (
              <path 
                ...
                stroke="#666" 
                strokeWidth="2" 
                markerEnd="url(#arrow-outreach)" 
              />
            )}
          </g>
        ))}
        <defs>
          <marker id="arrow-outreach" viewBox="0 0 10 10" refX="9" refY="5" markerWidth="6" markerHeight="6" orient="auto-start-reverse">
